...
globals [
  ...
  hive-number      ; number of hives
  hive-timer       ; a timer keeping track of the days elapsed at a hive
...
to setup-hives
  set probability 0.5 ; should be made variable in future
  set color-list [ 97.9 57.5 17.6 27.5 117.9 114.4 ]
  ;set color-list [ 97.9 57.5 17.6 27.5 117.9 ]
  set quality-list (list sales-a sales-b sales-c sales-d sales-e sales-f)
  ;set quality-list (list sales-a sales-b sales-c sales-d sales-e)
	; hives/quality list corresponds to initial quality as shown by dataset
...
to inspect-hive
  set inspect-hive-task [ ->
    ...
    ifelse bee-timer < 0 [
      ;ifelse hive-timer < 0 [
      ask target [
            set quality quality + (influence / 100) ;CHANGE LOG
            show ceiling quality
            set label ceiling quality
            set hive-timer 1200
            ;set visitors 0
    ]
      ;]
      ...
      ;set visitors visitors + count scouts with [ on-site? and target = [ target ] of myself ]
...
"" ""
PENS
"shilling" 1.0 0 -1398087 true "" "plot count scouts with [task-string = \"dancing\"]"
"apeing" 1.0 0 -7025278 true "" "plot count scouts with [task-string = \"discover\"] + count scouts with [task-string = \"re-visit\"]"
