...
scouts-own [
  ...
  inspecting-collection?         ; true if it's inspecting a collection collection

  ; tweet related variables:

  dist-to-collection     ; the distance between the swarm and the collection that a person is exploring
  circle-switch    ; when making a waggle tweet, a person alternates left and right to make
...
to setup-collections
  ...
  let i 0 ; assign num-sales and plot pens to each collection
  repeat count collections [
    ...
    set-current-plot "inspecting-collection"
...
to setup-people
  create-scouts 500 [ ;change to 500 - justify though...
...
to discover
  set discover-task [ ->
    ...
    [
      ; if a person finds collections around it (within a distance of 3) on its way
      ifelse count collections in-radius 15 > 0 [
        ; then randomly choose one to focus on
        let temp-target one-of collections in-radius 15
        let temp-location ([xcor ycor] of temp-target)
        ; distributes early-adopters near-equally
        ifelse count turtles-at temp-location < 2 [
          ...
          set interest [ num-sales ] of target
...
to inspect-collection
  set inspect-collection-task [ ->
    ...
    ; last comment of this task) on inspecting collections, they fly home.
    ifelse person-timer < 0 [
      ask target [
      ...
      if distance target > 2 [
        face target fd 1 ; a person flies to its target collection
      ]
      ...
      ; a person knows how far this collection is from its swarm
...
; time, they return to the swarm and tweet enthusiastically about it for a lengthy period
...
to tweet
  set tweet-task [ ->
      ...
      if person-timer > interest - (trips - 1) * (15 + random 5) and interest <= 0 [
        ...
        ; swarm, rest for 50 ticks, and then watch tweet
        set next-task watch-tweet-task
        set task-string "watching-tweet"
        set target nobody
        set interest 0
...
to go
  ...
  ; if all scouts are on collection, and they all have the same target collection, stop.
	(ifelse
    ticks < 16800 [ ;16800 is two weeks worth of ticks
...
to waggle
  ...
  ; switch toggles between 1 and -1, which makes a person
  ; tweet a zigzag line by turning left and right
  let waggle-switch 1
  ; first part of a zigzag line
  lt 60
  fd .4
  ; correlates the number of turns in the zigzag line with the distance
  ; between the swarm and the collection. the number 2 is selected by trial
  ; and error to make the tweet path look good
...
to plot-inspecting-collection-scouts
  ...
  repeat count collections [
    ...
    plot count scouts with [inspecting-collection? and target = collection i]
