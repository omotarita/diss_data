    ...
    while rw < len(df):
        ...
        for element in query_list:
            try:
                ...
                #API request
                result = client.search_recent_tweets(query = q, end_time = end, start_time = start)
                tweets = result[0]
                ...
                if tweets != None:
                    for x in range(len(tweets)):
                        if i < 100:
                            ...
                            result = client.get_tweet(id = id, tweet_fields = "public_metrics")
                            metrics = result[0]['public_metrics']
                            impressions = metrics['retweet_count'] + metrics['reply_count'] + metrics['like_count'] + metrics['quote_count']
                ...
                print(f"For {df_column} on {start}, the impression count is {total_impressions}")

                #Add to dataframe
                df.loc[rw, df_column] = total_impressions
...
def main():
    ...
    dataset_file = "impressions_files/tweet_impressions_template.csv"

    #Comment out once tested
    #dataset_file = "tweetcount_template_pastweek_copy.csv"

    nft_file = "nft_keywords2.csv"
    output_file = "impressions_files/tweet_impressions_data.csv"
